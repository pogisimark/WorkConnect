...
try {
    ...
    // Check if company user exists (ONLY in company_users table, not employee_users)
    $stmt = $conn->prepare("SELECT id, company_name, email FROM company_users WHERE email = ?");
    if (!$stmt) {
    ...
    if ($result->num_rows > 0) {
        ...
        // Create separate password_resets table for companies
        $create_table_sql = "CREATE TABLE IF NOT EXISTS company_password_resets (
        ...
        // Store reset token in database (company_password_resets table)
        $stmt = $conn->prepare("INSERT INTO company_password_resets (user_id, email, token, expires_at, created_at) VALUES (?, ?, ?, ?, NOW()) ON DUPLICATE KEY UPDATE token = VALUES(token), expires_at = VALUES(expires_at), created_at = NOW()");
...
<div style='max-width: 600px; margin: 0 auto; padding: 20px;'>
                    ...
                    <p>Hello " . htmlspecialchars($company['company_name']) . ",</p>
                    <p>You have requested to reset your password for your WorkConnect Company account.</p>
        ...
        sendResponse(true, 'If a company account with that email exists, we\'ve sent you a password reset link.');
    }
